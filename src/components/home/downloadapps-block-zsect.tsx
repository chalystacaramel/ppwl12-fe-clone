// @ts-nocheck
import { X78zum5 } from './x78zum5';

export const DownloadappsBlockZsect = () => {
  return (
    <>
      <div className="downloadApps_block__ZSEcT downloadApps_smallBlock__qc_vK downloadApps_smallBlockWithGap__aQQkF">
        <div className="downloadApps_blockContent__r_j_n downloadApps_smallBlockContent__eUpbA">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            width="1em"
            className="NotionIconSvgWrapper_icon__8quiY downloadApps_appIcon___8Q6r"
          >
            <svg
              className="forms"
              viewBox="0 0 57 60"
              style={{
                display: 'block',
                fill: 'inherit',
                flexShrink: '0',
                height: '100%',
                width: '100%',
              }}
            >
              <path
                d="M6.18613 49.1689C5.15664 49.1575 4.2418 48.8935 3.53102 48.23C3.53102 48.23 3.52873 48.23 3.52643 48.2254C3.44389 48.1451 3.36593 48.0625 3.29256 47.9798C2.73082 47.337 2.42587 46.5129 2.42587 45.6222L2.41899 8.93385C2.41899 6.81041 4.20741 4.90505 6.3237 4.77421L42.9289 2.50615C43.0091 2.50155 43.0871 2.49926 43.1673 2.49926C44.0822 2.49926 44.9351 2.82983 45.5863 3.44505C45.6757 3.52999 45.7605 3.61722 45.8385 3.70904C46.0494 3.95008 46.2237 4.21637 46.3613 4.50332C46.226 4.21867 46.0494 3.95238 45.8385 3.71134C46.3865 4.34952 46.6869 5.16216 46.6869 6.04368V6.98258C46.6869 6.98258 46.7098 7.75161 45.9486 7.80211L45.9531 7.8067L12.2851 9.91866C9.81569 10.0725 7.80946 12.2143 7.80946 14.6912C7.80946 14.6912 7.7957 48.5537 7.79341 48.6088C7.76819 49.1689 7.30733 49.1689 6.92672 49.1689H6.18613Z"
                fill="black"
              />
              <path
                d="M54.5994 52.0086C54.5994 52.0637 54.5948 52.1188 54.5902 52.1739C54.51 53.3515 53.5722 54.3892 52.4189 54.6026C52.396 54.6026 52.3731 54.6095 52.3501 54.6095L13.3008 56.9763H13.1792C12.4157 56.9373 11.7095 56.6434 11.1592 56.1246C10.5425 55.5438 10.1963 54.7496 10.171 53.8795C10.171 53.8451 10.1664 53.8107 10.1664 53.7739V14.9277C10.1664 14.8726 10.1664 14.8198 10.171 14.7647C10.2559 13.5182 11.2968 12.4301 12.5395 12.3015C12.5716 12.3015 12.6037 12.2923 12.6358 12.29L51.7058 9.83832C51.7677 9.83373 51.8296 9.83143 51.8916 9.83143C52.5955 9.83143 53.2512 10.0862 53.7533 10.5568C54.1041 10.8851 54.3518 11.296 54.4848 11.7529C54.5421 11.948 54.5765 12.1523 54.5902 12.3612L54.5994 52.0086Z"
                fill="black"
              />
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M48.9865 50.7782L16.2929 52.6835C14.9562 52.7776 13.7731 52.3231 13.8098 50.5693V23.2699C13.8098 22.3333 14.5802 21.7341 15.4308 21.6905L49.4199 19.6497C50.2682 19.6061 50.963 20.2213 50.963 21.073V48.4206C50.963 49.4215 50.713 50.7093 48.9934 50.7759H48.9888L48.9865 50.7782Z"
                fill="white"
              />
              <path
                d="M23.935 29.9662C22.3369 30.0649 21.7958 31.2862 21.7981 33.164V33.4945C21.6078 33.5405 21.4473 33.5818 21.2547 33.5933C20.1037 33.6644 19.2737 32.7278 19.2714 31.0474C19.2691 28.4763 21.5665 26.0591 25.9435 25.7859C29.8414 25.5426 32.336 27.4296 32.3406 30.7582C32.3406 33.2627 30.2678 35.0418 28.2249 35.5973C32.0585 35.7856 33.6589 37.9274 33.6635 40.6637C33.6681 45.3445 30.2518 47.9959 24.9805 48.3242L24.8521 48.3311C20.8901 48.579 18.1089 47.0708 18.1066 44.4675C18.1066 42.9846 19.1888 41.7312 20.819 41.6279C20.9474 41.6187 21.0735 41.6439 21.2019 41.6371C21.5252 44.2862 23.3159 45.2618 25.1044 45.1516C26.8286 45.0437 28.0415 43.8799 28.0415 42.2316V42.165C28.0392 39.594 25.8977 39.4654 22.6694 39.3369L22.1558 36.2699C25.1594 35.719 26.6589 34.7365 26.6566 32.9574C26.6566 31.0451 25.5996 29.8583 23.9373 29.9639H23.935V29.9662Z"
                fill="black"
              />
              <path
                d="M38.3432 28.7036C35.1492 29.6287 34.4453 28.2215 34.7938 26.75C36.7107 26.3001 41.693 24.8034 43.5777 24.1262L43.6007 43.8684L47.0835 44.3757C47.0835 45.6613 46.3498 46.4647 45.0406 46.5474C43.9537 46.6139 41.4293 46.7058 40.2164 46.7815C38.3317 46.8986 34.881 47.181 34.881 47.181C34.7847 46.9238 34.7526 46.6621 34.7526 46.4303C34.7526 45.8036 35.0071 45.1608 35.8371 44.8785L38.3592 44.0612L38.3409 28.7036H38.3432Z"
                fill="black"
              />
            </svg>
          </svg>
          <div>
            <h3 className="downloadApps_appName__LVtJy">Notion Calendar</h3>
            <p className="downloadApps_appTagline__bn_xm">
              Time, scheduling, tasks—all together.
            </p>
          </div>
          <a
            className="button_button__atjat button_buttonVariantTertiary__lrfOH button_buttonSizeM__NexGD"
            aria-label="Learn more about Notion Calendar"
            data-analytics-event="click_cta"
            href="https://www.notion.com/product/calendar/download"
          >
            Download
          </a>
        </div>
        <div className="downloadApps_blockAsset___CqLJ downloadApps_smallBlockAsset__oWidw">
          <div
            className="notion-light-theme"
            style={{
              display: 'contents',
            }}
          >
            <div
              className="ndsIslandContainer"
              style={{
                '--nds-h': '468',
                '--nds-w': '436',
              }}
            >
              <div
                className="ndsIslandScale"
                style={{
                  '--nds-h': '468px',
                  '--nds-scale': '0.6410256410256411',
                  '--nds-w': '436px',
                }}
              >
                <div
                  style={{
                    aspectRatio: '436/468',
                    backgroundColor: 'var(--c-bacPri)',
                    display: 'flex',
                    flexDirection: 'column',
                    fontFamily:
                      'ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI Variable Display","Segoe UI",Helvetica,"Apple Color Emoji","Noto Sans Arabic","Noto Sans Hebrew",Arial,sans-serif,"Segoe UI Emoji","Segoe UI Symbol"',
                    maxWidth: '436px',
                    overflow: 'hidden',
                    width: '436px',
                  }}
                >
                  <div
                    className="xh8yej3 x1obq294 x5a5i1n xde0f50 x15x8krk x1cnzs8 x1xnnf8n xx6bls6 x106a9eq xt2wqj3"
                    style={{
                      '--x-background': 'var(--c-bacPri)',
                    }}
                  >
                    <div
                      className="x78zum5 xdt5ytf"
                      style={{
                        gap: '16px',
                      }}
                    >
                      <div
                        style={{
                          fontSize: '17px',
                          fontWeight: '600',
                          lineHeight: '22px',
                        }}
                      >
                        Meeting with Stephanie Lee
                      </div>
                      <div
                        className="x78zum5"
                        style={{
                          alignItems: 'center',
                          gap: '10px',
                        }}
                      >
                        <div
                          style={{
                            alignItems: 'center',
                            backgroundColor: 'var(--c-bacTer)',
                            borderRadius: '50%',
                            display: 'flex',
                            flexShrink: '0',
                            height: '30px',
                            justifyContent: 'center',
                            overflow: 'hidden',
                            width: '30px',
                          }}
                        >
                          <img
                            alt="Stephanie Lee"
                            loading="lazy"
                            width="32"
                            height="32"
                            decoding="async"
                            data-nimg="1"
                            className="next-image"
                            srcSet=""
                            src="https://static.step1.dev/vogxpj/assets/c45710091feb.webp"
                            style={{
                              color: 'transparent',
                            }}
                            data-airgap-id="57"
                            sizes=""
                          />
                        </div>
                        <div
                          className="x78zum5 xdt5ytf"
                          style={{
                            gap: '0px',
                          }}
                        >
                          <div
                            style={{
                              fontSize: '12px',
                              fontWeight: '600',
                              lineHeight: '16px',
                            }}
                          >
                            Stephanie Lee
                          </div>
                          <div
                            style={{
                              color: 'var(--c-texTer)',
                              fontSize: '12px',
                              fontWeight: '400',
                              lineHeight: '16px',
                            }}
                          >
                            Organizer
                          </div>
                        </div>
                      </div>
                      <div
                        style={{
                          alignItems: 'center',
                          borderColor: 'var(--c-borPri)',
                          borderRadius: '4px',
                          borderStyle: 'solid',
                          borderWidth: '1px',
                          display: 'flex',
                          justifyContent: 'space-between',
                          paddingBlock: '4.5px',
                          paddingInline: '8px',
                        }}
                      >
                        <div
                          className="x78zum5"
                          style={{
                            alignItems: 'center',
                            gap: '12px',
                          }}
                        >
                          <svg
                            aria-hidden="true"
                            role="graphics-symbol"
                            viewBox="0 0 20 20"
                            className="globe x1lliihq x2lah0s x1kky2od xlup9mm x19zyb68 x14rh7hd"
                            style={{
                              '--x-color': 'var(--c-icoSec)',
                              '--x-fill': 'var(--c-icoSec)',
                            }}
                          >
                            <path d="M10 2.375a7.625 7.625 0 1 1 0 15.25 7.625 7.625 0 0 1 0-15.25m-1.863 8.25c.054 1.559.31 2.937.681 3.943.212.572.449.992.68 1.256.232.266.404.318.502.318s.27-.052.502-.318c.231-.264.468-.684.68-1.256.371-1.006.627-2.384.681-3.943zm-4.48 0a6.38 6.38 0 0 0 4.509 5.48 6.5 6.5 0 0 1-.52-1.104c-.431-1.167-.704-2.697-.76-4.376zm9.456 0c-.055 1.679-.327 3.21-.758 4.376-.15.405-.324.779-.522 1.104a6.38 6.38 0 0 0 4.51-5.48zM8.166 3.894a6.38 6.38 0 0 0-4.51 5.481h3.23c.056-1.679.328-3.21.76-4.376.15-.405.322-.78.52-1.105M10 3.858c-.099 0-.27.053-.502.319-.231.264-.468.683-.68 1.255-.371 1.006-.627 2.384-.681 3.943h3.726c-.054-1.559-.31-2.937-.681-3.943-.212-.572-.449-.99-.68-1.255-.232-.266-.404-.319-.502-.319m1.833.036c.198.326.372.7.521 1.105.432 1.167.704 2.697.76 4.376h3.23a6.38 6.38 0 0 0-4.511-5.481" />
                          </svg>
                          <div
                            className="x78zum5"
                            style={{
                              alignItems: 'center',
                              gap: '4px',
                            }}
                          >
                            <div
                              style={{
                                color: 'var(--c-texTer)',
                                fontSize: '12px',
                                fontWeight: '400',
                                lineHeight: '16px',
                              }}
                            >
                              GMT+7
                            </div>
                            <div
                              style={{
                                fontSize: '12px',
                                fontWeight: '500',
                                lineHeight: '16px',
                              }}
                            >
                              Bangkok
                            </div>
                          </div>
                        </div>
                        <div
                          style={{
                            color: 'var(--c-texTer)',
                            fontSize: '12px',
                            fontWeight: '400',
                            lineHeight: '16px',
                          }}
                        >
                          4:29 AM
                        </div>
                      </div>
                      <div
                        className="x78zum5"
                        style={{
                          alignItems: 'flex-start',
                          gap: '24px',
                        }}
                      >
                        <X78zum5 />
                        <div className="x78zum5 x1iyjqo2" />
                        <div
                          style={{
                            display: 'flex',
                            flexDirection: 'column',
                            gap: '15px',
                            width: '144px',
                          }}
                        >
                          <div
                            style={{
                              fontSize: '12px',
                              fontWeight: '500',
                              lineHeight: '16px',
                            }}
                          >
                            Wednesday, Apr 29
                          </div>
                          <div
                            className="x78zum5 xdt5ytf"
                            style={{
                              gap: '12px',
                            }}
                          >
                            <div
                              role="button"
                              tabIndex="0"
                              className="x87ps6o x1b7c0jy x1ypdohk x3nfvp2 x6s0dn4 xl56j7k xuxw1ft xk50ysn x1u7k74 x1fgtraw xmzvs34 xf159sx x1i5p2am x1whfx0g xr2y4jy x1ihp6rs xif65rj x1ha54eg x1ym9u1z x1gat650 x1t8z4yu x14rh7hd x1md70p1 x1f1i3pa x1iazond xayv9eh x1811ece xigsv13"
                              fdprocessedid="1fwdwe"
                              style={{
                                '--x-13727ff':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 1.5)))',
                                '--x-8zloot': 'var(--ca-bluBacSecTra)',
                                '--x-border':
                                  '1px solid var(--ca-bluBorPriTra)',
                                '--x-color': 'var(--c-bluTexAccPri)',
                                '--x-ovxak1':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 1.5)))',
                                '--x-ph3bdx':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 0.5)))',
                                '--x-pjywqs': 'var(--ca-bluBacSecTra)',
                                height: '38px',
                              }}
                            >
                              2:00pm
                            </div>
                            <div
                              role="button"
                              tabIndex="0"
                              className="x87ps6o x1b7c0jy x1ypdohk x3nfvp2 x6s0dn4 xl56j7k xuxw1ft xk50ysn x1u7k74 x1fgtraw xmzvs34 xf159sx x1i5p2am x1whfx0g xr2y4jy x1ihp6rs xif65rj x1ha54eg x1ym9u1z x1gat650 x1t8z4yu x14rh7hd x1md70p1 x1f1i3pa x1iazond xayv9eh x1811ece xigsv13"
                              fdprocessedid="j2zb1n"
                              style={{
                                '--x-13727ff':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 1.5)))',
                                '--x-8zloot': 'var(--ca-bluBacSecTra)',
                                '--x-border':
                                  '1px solid var(--ca-bluBorPriTra)',
                                '--x-color': 'var(--c-bluTexAccPri)',
                                '--x-ovxak1':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 1.5)))',
                                '--x-ph3bdx':
                                  'rgb(from var(--ca-bluBacSecTra) r g b/calc(alpha*(1 + 0.5)))',
                                '--x-pjywqs': 'var(--ca-bluBacSecTra)',
                                height: '38px',
                              }}
                            >
                              5:00pm
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
